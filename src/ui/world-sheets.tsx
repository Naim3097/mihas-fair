...
export function MapSheet({ engine }: Eng) {
  ...
  const tap = (e: MouseEvent) => {
    ...
    if (booth && booth.k < 3.5) return booth.b.id === lv.hero.id ? go(lv.hero.dock.x, lv.hero.dock.y, 'The X · Booth 7E17') : go(booth.b.x, booth.b.y, name(booth.b));
      ...
      {hits ? (
        ...
        <>
          ...
          <div class="results flow" style={{ marginTop: '12px' }}>
            {deck === 2 && <button class="result hero" onClick={() => go(lv.hero.dock.x, lv.hero.dock.y, 'The X · Booth 7E17')}><strong>The X — leanxdigital</strong><small>Booth 7E17 · Hall 7 · your tote bag when the checkpoints are done</small></button>}
...
export function PhotoSheet() {
  ...
  return (
    <Sheet k="Photo" title="You, at MIHAS 2026">
      ...
      <div class="stack">
        {canShare && <button class="btn primary big" onClick={async () => { try { await navigator.share({ files: [await file()], title: 'Mission X at MIHAS 2026', text: 'Find the X — Booth 7E17, MIHAS 2026.' }); } catch { /* closed the share sheet */ } }}>Share</button>}
