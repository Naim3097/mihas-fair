...
export interface FairSink {
  ...
  onKey(action: 'interact' | 'wave' | 'cheer' | 'dance' | 'map'): void;
  /** false while a sheet is open: the world does not listen then */
  enabled(): boolean;
}
...
export class FairInput {
  /** Desired move in screen space: x right, y forward, length ≤ 1. */
  ...
  constructor(private el: HTMLElement, private sink: FairSink) {
  ...
  private down(e: PointerEvent) {
    try { this.el.setPointerCapture(e.pointerId); } catch { /* the pointer is already gone */ }
    ...
    if (this.drags.size === 2) { this.pinch = this.pinchDist(); for (const d of this.drags.values()) d.moved = true; }
  }
  ...
  private moveEv(e: PointerEvent) {
    ...
    if (s?.id === e.pointerId) {
      ...
      const mag = l <= STICK_DEAD ? 0 : (l - STICK_DEAD) / (STICK_R - STICK_DEAD);
    ...
    else if (d.moved) this.sink.onOrbit(dx, dy);
  }
