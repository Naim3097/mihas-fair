...
// geometry is the fair's own (src/fair/stands.ts, pure plan maths), read at the same 3 m module the world draws, so
...
import { fairLevelData } from '../src/fair/level.js';
import { DIR, counterOf, planStands } from '../src/fair/stands.js';
...
export function counterSpots(level: LevelData): Map<string, CounterSpot> {
  ...
  for (const st of planStands(fairLevelData(level))) {
