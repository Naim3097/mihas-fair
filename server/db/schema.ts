...
-- ---------------------------------------------------------------- Ceritera
-- The library game's character: one per player, the avatar class they chose, and their XP. Level and stats are
-- computed from xp by the content package (content/progression.ts), never stored, so no runtime can disagree.
...
-- Every XP change, append-only. characters.xp is a cache of the sum, written in the same batch.
CREATE TABLE IF NOT EXISTS ceritera_ledger (
  id          INTEGER PRIMARY KEY AUTOINCREMENT,
  player_id   TEXT NOT NULL REFERENCES players(id),
  action      TEXT NOT NULL,
  xp          INTEGER NOT NULL,
