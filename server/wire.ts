...
export interface Services { game: Game; stations: Stations; social: Social; crews: Crews; venue: Venue; director: Director; gc: GroundControl; ops: LiveOps; signer: Signer; /** the library game's characters */ ceritera: Ceritera }
...
export function buildServices(o: { db: Db; secret: string; level: LevelData; publicOrigin: string; venue?: VenueConfig; now?: () => number; /** defaults to in-memory; pass DbPresence on serverless */ presence?: Presence; /** switched-off systems to run anyway (their tests do) */ features?: Partial<Features> }): Services {
  ...
  const ceritera = new Ceritera(o.db, o.now);
  ...
  return { game, stations, social, crews, venue, director, gc, ops, signer, ceritera };
}
